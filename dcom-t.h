...
/* Rev 16 Dec 2000	narrow band filter for clock rate of 11,025 Hz
 * introduced */
...
#if(frevt >= 23)
  #define is1len                                   \
    24 /*; number of sample points to use for each \
       ; stage 1 interpolated point*/
  #define is2len                                   \
    8  /*; number of sample points to use for each \
...
#define da_offstg2                                  \
  ((is2len / 2) - 1) /*; in[da_offstg2] corresponds \
...
#define ipts                              \
  (fap / INFx) /* # of points to generate \
...
/* interpolation stage 1 offset + extra sample(s) needed for stage 1
 * interpolation */
...
#define s1off                                \
  ((s0off - ds1dly) / ds1dcf) /* offset into \
      stage 1 array of reference point */
#define s2off                                \
  ((s1off - ds2dly) / ds2dcf) /* offset into \
      stage 2 array of reference point */
#define s3off                                \
  ((s2off - ds3dly) / ds3dcf) /* offset into \
...
#define iptsnb2                              \
  (fapnb2 / INFx) /* # of points to generate \
                  interpolated values for */
...
/* offset into initial array of reference point   */
#define s1nb2off                                      \
  ((s0nb2off - ds1nb2dly) / ds1nb2dcf) /* offset into \
       stage 1 array of reference point */
#define s2nb2off                                      \
  ((s1nb2off - ds2nb2dly) / ds2nb2dcf) /* offset into \
       stage 2 array of reference point */
#define s3nb2off                                      \
  ((s2nb2off - ds3nb2dly) / ds3nb2dcf) /* offset into \
       stage 3 array of reference point */
