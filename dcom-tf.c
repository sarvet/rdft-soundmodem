...
5X stage of 5X-4X-2X decimator for phase modulation version of digital HF
signal, prior to transmission.  Overall filter: passes 0-85.546 Hz	with a
gain of 2, stops > 137.8125 Hz. Filter Length =  17
********* IMPULSE RESPONSE *********$#**/
ftype ds1cf[] = {
...
4X stage of 5X-4X-2X decimator for phase modulation version of digital HF
signal, prior to transmission.  Overall filter: passes 0-85.546 Hz	with a
gain of 2, stops > 137.8125 Hz. Filter Length =  19
********* IMPULSE RESPONSE *********$#**/
ftype ds2cf[] = {
...
2X stage of 5X-4X-2X decimator for phase modulation version of digital HF
signal, prior to transmission.  Overall filter: passes 0-85.546 Hz	with a
gain of 2, stops > 137.8125 Hz. Filter Length =  29
********* IMPULSE RESPONSE *********$#**/
ftype ds3cf[] = {
...
void dfilter(ftype *in, ftype *out, unsigned int count, unsigned int delay,
             ftype *coef)
{
  ftype        acum;
  register int si, k;
...
*/
void ddfilter(ftype *in, ftype *out, unsigned int count, unsigned int delay,
              unsigned int dcf, ftype *coef)
{
  ftype        acum;
  register int si, k;
  unsigned int j;
 ...
 **/
  ...
  si   = -(da_offstg2);
  acum = 0.0;
  for(di = 0; di < is2len; di++)
  {
    acum += in[si] * is2cI[cf][di];
    si++;
  }
  return acum;
}
 ...
 **/
  ...
  for(si = 0; si < num_sp; si++)
  {
    ...
    out[oc] = x2[d1];
    oc++;
    for(frac = 1; frac < is2inf; frac++)
    {
      out[oc] = I20xxPI(frac, &x2[d1]);
      oc++;
    }
    d1++;
    out[oc] = x2[d1];
