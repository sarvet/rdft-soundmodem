...
{unsigned int	i;
 ...
 for (i=0; i<=(dft_siz/4); i++) snqt[i]=(float)(sin(sstp*i));
 return((double)(1.0/(t_step*dft_siz)));
}

void	dfft3f(float *inr, float *ini, float *outr, float *outi,
...
{/* inr		points to real part of input
 ...
 for (I=0; I<=N; I++) *(outr+I) = *(inr+I);
 if ((real_in==1)&&(inv!=1)) {for (I=0; I<=N; I++) *(outi+I) = 0.0;}
 else {for (I=0; I<=N; I++) *(outi+I) = *(ini+I);}
 ...
 {S2= *(sint+M);
  if (inv!=1) S2=-S2;
  S1= *(sint+NQ-M);
  if (J>=IR) {M=M-IT; S1=-S1;}
  else M=M+IT;
  for (ND=J; ND<=NT; ND+=ID)
  {K=ND; L=K+I;
  /* butterfly computation */
   R1r=*(outr+L); 	R1i=*(outi+L);
   Rr= S1*R1r - S2*R1i;	Ri= S1*R1i + S2*R1r;
   R1r=*(outr+K);	R1i=*(outi+K);
   *(outr+K)= R1r + Rr;	*(outi+K)= R1i + Ri;
   *(outr+L)= R1r - Rr;	*(outi+L)= R1i - Ri;
  }
 }
 IR=I;
 IT=NQ/I;
