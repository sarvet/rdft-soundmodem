...
char ins[]
    = "Useage: flt-clip-scale-wav in-file out-file clipping_level "
      "scale_factor\n";
...
char *erexmsg[] = { noerr, ins,  ukner, em03, em04, em05, em06,
                    em07,  em08, em09,  em10, em11, em12, em13 };
...
int main(int argc, char *argv[], char *env[])
{
  ...
  ERN    = 8;
  if((fbufin = malloc((size_t) ((numel) * sizeof(in_type)))) == NULL)
    goto exa;
  if((fbufot = malloc((size_t) ((numel) * sizeof(out_type)))) == NULL)
    goto exb;
  ERN = 3;
  inf = fopen(argv[1], "rb");
  if(inf == NULL)
  ...
  if(val_written != (size_t) (wav_len1_off))
    goto exe;
  val_written = fwrite(&wav_bc1, sizeof(unsigned int), 1, otf);
  if(val_written != (size_t) (1))
    goto exe;
  val_written = fwrite(&wav_hdr_buf[wav_len1_off + sizeof(unsigned int)], 1,
                       wav_len_diff - sizeof(unsigned int), otf);
  if(val_written != (size_t) (wav_len_diff - sizeof(unsigned int)))
    goto exe;
