/*
    Reed-Solomon (8,4) code over GF(9), used by Wyman1x digital communications
   programs Copyright (C) 2001  Barry Sanderson
 ...
 */
  ...
  for(si = 0; si < n2; si++)
    if((codeword[si] < 0) || (codeword[si] > n2))
      return -1;
  if(rho > (2 * t2))
    return -2;
  if(rho > 0)
  ...
  */
  while(r < rho)
  {
    ...
    {
      for(si = n2 - 1; si > 0; si--)
        sh[si] = gf9mul[expa2[erase[r - 1]]][lam[si - 1]];
      sh[0] = 0;
      for(si = 0; si < n2; si++)
      {
        lam[si] = gf9add[lam[si]][gf9addi[sh[si]]];
        B[si]   = lam[si];
      }
      L++;
    }
  ...
  while(r < 2 * t2)
  {
    ...
    /* calculate del[r] */
    tmpi = 0;
    for(si = 0; si <= (r < n2 ? r : n2 - 1); si++)
      tmpi = gf9add[tmpi][gf9mul[lam[si]][S[(r - si) % n2]]];
    del[r % n2] = tmpi;
    while(del[r % n2] < 0)
      del[r % n2] += N2;
    if(del[r] == 0)
    ...
    {
      for(si = 1; si < n2; si++)
        sh[si] = gf9mul[del[r % n2]][B[si - 1]];
      sh[0] = 0;
      for(si = 0; si < n2; si++)
        T[si] = gf9add[lam[si]][gf9addi[sh[si]]];
      if(2 * L > r + rho - 1)
      {
        for(si = 0; si < n2; si++)
          lam[si] = T[si];
